...
//
// }
class ColombeRecord extends StatefulWidget {
  ColombeRecord({super.key});
  DatabaseHelper db = DatabaseHelper();



  @override
  State<ColombeRecord> createState() => _ColombeRecordState();
}

class _ColombeRecordState extends State<ColombeRecord> {
  ...
  Future<List<Members>> getMembers() async {
    ...
    List<Members> membersList = await dbHelper.colombe();
  ...
  DataRow recentFileDataRow(Members member, int count) {
    ...
    return DataRow(
      cells: [
        ...
        DataCell(Text(calculateAgeFromDateString(member.dateOfBirth!) ?? 'Unknown')),
  ...
  String? calculateAgeFromDateString(String dateOfBirth) {
    try {
      final dateFormat = DateFormat('dd-MM-yyyy'); // Adjust format as needed
      final birthDate = dateFormat.parse(dateOfBirth);
